...
public class UserInvitationMgtServiceHolder {
    ...
    public static InvitationCoreService getInvitationCoreService() {
        ...
        if (log.isDebugEnabled()) {
            log.debug("Retrieving InvitationCoreService OSGi service instance.");
        }
        InvitationCoreService service = InvitationCoreServiceHolder.SERVICE;
        if (service == null) {
            log.warn("InvitationCoreService OSGi service is not available. Service may not be initialized.");
        }
        return service;
    }
}
