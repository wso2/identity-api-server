...
public class KeyStoreManagamentDataHolder {
    ...
    public static KeyStoreManagementService getKeyStoreManager() {
        ...
        if (log.isDebugEnabled()) {
            log.debug("Retrieving KeyStoreManagementService from OSGi service registry.");
        }
        KeyStoreManagementService keyStoreManagementService = KeyStoreManagementServiceHolder.SERVICE;
        if (keyStoreManagementService == null) {
            log.warn("KeyStoreManagementService is not available in the OSGi service registry.");
        }
        return keyStoreManagementService;
    }
}
