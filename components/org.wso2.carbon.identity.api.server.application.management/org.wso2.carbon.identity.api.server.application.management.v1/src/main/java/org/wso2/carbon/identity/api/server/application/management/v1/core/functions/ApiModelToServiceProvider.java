...
public class ApiModelToServiceProvider implements Function<ApplicationModel, ServiceProvider> {
            ...
            Optional.ofNullable(provisioningConfigApiModel.getOutboundProvisioningIdps())
                    .ifPresent(idps -> {
                        ...
                        IdentityProvider[] identityProviders =
                                idps.stream().map(new ApiModelToProvisioningIdp()).toArray(IdentityProvider[]::new);
    ...
    private void updateSubjectClaimConfigs(SubjectConfig subject, ServiceProvider application) {
        ...
        claimConfig.setAlwaysSendMappedLocalSubjectId(Utils.getBooleanValue(subject.getUseMappedLocalSubject()));

        LocalAndOutboundAuthenticationConfig authConfig = getLocalAndOutboundConfig(application);
        authConfig.setUseTenantDomainInLocalSubjectIdentifier(Utils.getBooleanValue(subject.getIncludeTenantDomain()));
        authConfig.setUseUserstoreDomainInLocalSubjectIdentifier(Utils.getBooleanValue(subject.getIncludeUserDomain()));
    }
    ...
    private void updateRoleClaimConfigs(RoleConfig roleApiModel, ServiceProvider application) {
        ...
        LocalAndOutboundAuthenticationConfig localAndOutboundConfig = getLocalAndOutboundConfig(application);
        localAndOutboundConfig.setUseUserstoreDomainInRoles(Utils.getBooleanValue(roleApiModel.getIncludeUserDomain()));
    }
    ...
    private void addAdvancedConfigurationToApplication(AdvancedApplicationConfiguration config,
        ...
        if (config != null) {
            ...
            setSpProperty(serviceProvider, IdentityConstants.SKIP_CONSENT, String.valueOf(config.getSkipConsent()));
    ...
    private void updateCertificate(Certificate certificate, ServiceProvider serviceProvider) {
        ...
        if (certificate != null) {
            if (certificate.getType() == Certificate.TypeEnum.PEM) {
                ...
                setSpProperty(serviceProvider, IdentityApplicationConstants.JWKS_URI_SP_PROPERTY_NAME,
                        certificate.getValue());
            }
        }
    }

    private void setSpProperty(ServiceProvider application, String key, String value) {

        ServiceProviderProperty[] spProperties = application.getSpProperties();
        ServiceProviderProperty[] newServiceProviderProperties;
        if (spProperties != null) {
            newServiceProviderProperties = Arrays.copyOfRange(spProperties, 0, spProperties.length + 1);
        } else {
            newServiceProviderProperties = new ServiceProviderProperty[1];
        }

        ServiceProviderProperty propertyToAdd = new ServiceProviderProperty();
        propertyToAdd.setDisplayName(key);
        propertyToAdd.setName(key);
        propertyToAdd.setValue(value);

        newServiceProviderProperties[newServiceProviderProperties.length - 1] = propertyToAdd;
        application.setSpProperties(newServiceProviderProperties);
    }
