...
public class IdleAccountIdentificationServiceHolder {
    ...
    public static IdleAccountIdentificationService getIdleAccountIdentificationService() {
        ...
        if (LOG.isDebugEnabled()) {
            LOG.debug("Retrieved IdleAccountIdentificationService: " + 
                    (service != null ? "Available" : "Null"));
        }
        if (service == null) {
            LOG.warn("IdleAccountIdentificationService is not available in the OSGi context");
        }
        return service;
    }
}
