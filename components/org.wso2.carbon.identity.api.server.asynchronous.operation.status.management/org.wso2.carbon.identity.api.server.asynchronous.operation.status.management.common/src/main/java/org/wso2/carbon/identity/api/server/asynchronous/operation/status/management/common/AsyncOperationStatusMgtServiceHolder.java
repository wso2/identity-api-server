...
public class AsyncOperationStatusMgtServiceHolder {
    ...
    public static AsyncOperationStatusMgtService getAsyncOperationStatusMgtService() {
        ...
        if (log.isDebugEnabled()) {
            log.debug("Retrieving AsyncOperationStatusMgtService OSGi service instance.");
        }
        AsyncOperationStatusMgtService service = ServiceHolder.SERVICE;
        if (service == null) {
            log.warn("AsyncOperationStatusMgtService OSGi service is not available. Service may not be initialized.");
        }
        return service;
    }
}
